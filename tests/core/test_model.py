...
def test_model():
    ...
    train_z_data = trainZmodel(np.linspace(0., 3., 301), np.ones(301), 1.5)
    model = Model(array_data, 'dummy', 0, dict(alice='bob'))
    model_2 = Model(array_data, 'dummy', 0)

    pickle_dump(array_data, 'array_raw.pickle')
    read_array_data = Model.read('array_raw.pickle', 'dummy', 0, dict(alice='bob'))
    read_array_data_2 = Model.read('array_raw.pickle', 'dummy', 0)
    ...
    check_dict_data = Model.read('dict_data.pickle')
    check_train_z_data = Model.read('train_z_data.pickle')
    check_model_data = Model.read('model_data.pickle')
