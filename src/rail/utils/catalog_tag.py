...
class CatalogTag(Configurable):
    ...
    config_options = dict(
        name=Param(str, None, required=True, msg="Tag to associate to this Catalog type"),        
        mag_column_template=Param(str, None, required=True, msg="Template to construct band magnitude column names"),
        mag_err_column_template=Param(str, None, required=True, msg="Template to construct band magnitude error column names"),
        filter_template=Param(str, None, required=True, msg="Template to filter file names"),
        ...
        hdf5_groupname=Param(str, "", msg="HDF5 group with photometry information, blank for top-level"),
    ...
    def _build_shared_params(self) -> dict:
        ...
        for band_name in self.config.band_list:
            ...
            if  'mag_err_column_name' in band_info:
                ...
                mag_err_column_name = self.config.mag_err_column_template.format(band=band_name)
    ...
    def apply(cls, tag: str) -> None:
        ...
        if tag not in cls._tag_dict:
            raise KeyError(f"Did not find tag: {tag} in known CatalogTags: {list(cls._tag_dict.keys())}")        
        cls._active_tag = tag
