...
class DataHandle:  # pylint: disable=too-many-instance-attributes
    ...
    def write(self, **kwargs: Any) -> None:
        ...
        if self.path is None:
            raise ValueError(
                "DataHandle.write() called but path has not been specified"
            )
        if self.data is None:
            raise ValueError(
                f"DataHandle.write() called for path {self.path} with no data"
            )
...
class TableHandle(DataHandle):
    ...
    def _write(cls, data: TableLike, path: str, **kwargs: Any) -> None:
        ...
        if cls.suffix is None:
            raise TypeError(
                "You are trying to use a TableHandle to write data,"
                "but that does not specify what format to write the data in.  "
                "Use a type-specfic sub-class such as PqHandle or Hdf5Handle.  "
                "to write data.  (It is ok to use TableHandle to read data.)"
            )
...
def pickle_model_read(modelfile: str) -> ModelLike:
...
def pickle_model_write(model: ModelLike, path: str) -> None:
...
class ModelHandle(DataHandle):
    ...
    default_model_read = pickle_model_read
    default_model_write = pickle_model_write
    
    @classmethod
    def _model_open(cls, path: str, mode: str, **kwargs: Any) -> FileLike:
    ...
    def _model_write(
        ...
        **_kwargs: Any,
        ...
        """Write the model, this default implementation uses pickle"""
        if writer is None:
            writer = cls.default_model_write
    ...
    def _open(cls, path: str, **kwargs: Any) -> FileLike:
        ...
        if kwcopy.pop("mode", "r") == "w":
            return cls._model_open(path, mode="wb", **kwcopy)
        reader = kwargs.pop("reader", None)
        return cls._model_read(path, reader=reader, **kwargs)
    ...
    def _write(cls, data: ModelLike, path: str, **kwargs: Any) -> None:
        ...
        return cls._model_write(data, path, force=force, writer=writer, **kwargs)
